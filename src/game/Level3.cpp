...
//play npc emote
bool ChatHandler::HandleNpcEmoteCommand(const char* args)
{
	...
	uint64 guid = target->GetDBTableGUIDLow();
    target->SetUInt32Value(UNIT_NPC_EMOTESTATE, emote);

	if (!sObjectMgr.AddNPCEmote(guid, emote))
	{
		SetSentErrorMessage(true);
		return false;
	}
...
bool ChatHandler::HandleNpcPathCommand(const char* args)
{
	if (!*args)
		return false;
	...
	uint32 point = atoi((char*)args);
	uint64 id = target->GetDBTableGUIDLow();

	PathforNPC path;
	path.position_x = player->GetPositionX();
	path.position_y = player->GetPositionY();
	path.position_z = player->GetPositionZ();
	path.orientation = player->GetOrientation();
	path.point = point;
	path.guid = id;
	
	if (!sObjectMgr.AddNPCPath(path))
	{
		SetSentErrorMessage(true);
		return false;
	}
